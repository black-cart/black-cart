...
theme = {
  ...
  initDashboardPageCharts: function() {
      ...
      tooltips: {
        callbacks: {
          label : function(tooltipItem, data) {
            let label = data.datasets[tooltipItem.datasetIndex].label[0];
            let value = data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index];
            let label1 = data.datasets[tooltipItem.datasetIndex].label[1];
            let value1 = data.datasets[tooltipItem.datasetIndex].data1[tooltipItem.index];
            if (value1 == '') {
              value1 = 0;
            }
            return [label + ':' + value, label1 + ':' + value1];
          }
        },
        backgroundColor: '#f5f5f5',
        titleFontColor: '#333',
    ...
    var data_k = $('#0').attr('data-k');
    var datatt_v = $('#0').attr('datatt-v');
    var dataod_v = $('#0').attr('dataod-v');
    data_k = $.parseJSON(data_k);
    datatt_v = $.parseJSON(datatt_v);
    dataod_v = $.parseJSON(dataod_v);
    ...
    $("#0").click(function() {
      $('.ttl-chart').html('In 12 Months');
      var data = myChartData.config.data;
      data.datasets[0].data = datatt_v;
      data.datasets[0].data1 = dataod_v;
      data.labels = data_k;
      myChartData.update();
    });

    $("#1").click(function() {
      $('.ttl-chart').html('In 30 days');
      var data_k = $(this).attr('data-k');
      var datatt_v = $(this).attr('datatt-v');
      var dataod_v = $(this).attr('dataod-v');
      data_k = $.parseJSON(data_k);
      datatt_v = $.parseJSON(datatt_v);
      dataod_v = $.parseJSON(dataod_v);
  ...
  initVectorMap: function() {
    ...
    var mapName = {};
    var mapAmount = {};
    x.forEach(function (key, i) {
      mapOrder[key.name] = key.order;
      mapName[key.name] = key.name;
      mapAmount[key.name] = key.amount;
    });
    $('#worldMap').vectorMap({
      map: 'world_mill_en',
      ...
      onRegionTipShow: function(e, el, code){
        if(mapOrder[code]){
          el.html(mapName[code]+'</br> Amount: '+mapAmount[code]+'</br> Order: '+mapOrder[code]);
        }else{
          el.html(mapName[code]);
        }
      }
    });
  },
