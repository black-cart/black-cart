<?php
    ...
    if(bc_config('ProductSale')) {
        ...
        */
        if (!function_exists('bc_product_flash_check_over')) {
            function bc_product_flash_check_over($pId, $qty) {
        ...
        if (!function_exists('bc_product_flash_update_stock')) {
            function bc_product_flash_update_stock($pId, $qty) {
                $product = (new \App\Plugins\Other\ProductSale\Models\PluginModel)
                    ->where('product_id', $pId)
                    ->first();
                if ($product) {
        ...
        if (!function_exists('bc_product_flash')) {
            function bc_product_flash($limit = 8, $paginate = false) {
         ...
         */
        if (!function_exists('bc_public_path_flash')) {
            function bc_public_path_flash($file) {
